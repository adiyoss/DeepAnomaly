...
X = create_sequences(db[:-1], win_size=maxlen, step=maxlen)
...
prediction_container = np.array(prediction_container).flatten()
Y = db[1:len(prediction_container)]
plt.plot(prediction_container, label='prediction')
